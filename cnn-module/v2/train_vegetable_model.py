...
for epoch in range(EPOCHS):
    ...
    if test_acc > best_acc + min_delta:
        ...
        print(f"✅ 标签映射已保存到 result_model/label_to_idx.json 和 result_model/idx_to_label.json")
    else:
        patience_counter += 1
...
torch.onnx.export(
    ...
    output_names=['output'],
    dynamic_axes={
        'input': {0: 'batch_size', 2: 'height', 3: 'width'},  # 支持动态批次、高度和宽度
        'output': {0: 'batch_size'}
    }
)
print("✅ 模型已成功导出为 ONNX 格式: result_model/vegetable_classifier.onnx")
print("   模型支持动态输入尺寸，部署时可以处理不同大小的图像")

# 显示保存的文件信息
print("\n保存的文件列表:")
print("- result_model/CNN_Vegetable_Model_best.pt (最佳模型权重)")
print("- result_model/label_to_idx.json (标签到索引的映射)")
print("- result_model/idx_to_label.json (索引到标签的映射)")
print("- result_model/vegetable_classifier.onnx (ONNX格式模型 - 支持动态输入)")
print("- result_images/training_analysis.png (训练分析图表)")

# 额外说明
print("\n" + "=" * 60)
print("模型部署说明:")
print("1. ONNX模型支持动态输入尺寸 (batch_size, channels, height, width)")
print("2. 在推理时，图像会被自动调整到224x224尺寸进行预测")
print("3. 可以处理任意尺寸的输入图像，模型会自动预处理")
print("=" * 60)
